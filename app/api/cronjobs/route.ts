    ...
    if(deals && deals.length > 0){
        for(let i=0;i<deals.length;i++){
            ...
            if(campaign && maindata){
                ...
                const updateAmountUsed = Math.round(campaign[0].amountUsed + deals[i].prices*( deals[i].pageAverageViews-maindata.views) * 0.0001);
                await updateCampaign(deals[i].sponId, updateAmountUsed);
                const updatedCredit=Math.round(deals[i].prices*maindata.views*0.0001);
