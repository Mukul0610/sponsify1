...
const PopularSponsorships = async() => {
  const allCampaigns = await getAllCampaigns();
  const popularSponsorship = allCampaigns.sort((a:any, b:any) => b.creditUseForCampaign - a.creditUseForCampaign)
  return (
    <div>
      <h2 className='text-3xl font-bold ml-4 lg:ml-0 text-neutral-800'>Popular Sponsorships</h2>
      
      
      <Carousel opts={{align: "end",}} className="w-[78%] py-2 lg:w-full md:w-full">
      <CarouselContent>
      {popularSponsorship.map((campaign: any) => (
...
export default PopularSponsorships